...
class ARCH_mnist():
	...
	def generator_model_mnist(self):
		...
		# noise_den = layers.Dense(int(self.output_size/4)*int(self.output_size/4)*255, use_bias=False,kernel_initializer=init_fn)(noise_ip)
		# # noise_den = layers.BatchNormalization()(noise_den)
		# # noise_den = layers.LeakyReLU()(noise_den)
		# noise_res = layers.Reshape((int(self.output_size/4), int(self.output_size/4), 255))(noise_den)

		# # this will be our label
		# image_class = tf.keras.Input(shape=(1,), dtype='int32')

		# # class_embed = tf.keras.layers.Embedding(input_dim = self.num_classes, output_dim = 50, embeddings_initializer='glorot_normal')(image_class)
		...
		# class_den = layers.Dense(int(self.output_size/4)*int(self.output_size/4), use_bias=False,kernel_initializer=init_fn)(image_class)
		# class_res = layers.Reshape((int(self.output_size/4), int(self.output_size/4), 1))(class_den)
	...
	def discriminator_model_mnist(self):
		...
		# model.add(layers.Conv2D(128, (5, 5), strides=(1, 1), padding='same', kernel_initializer=init_fn, input_shape=[int(self.output_size), int(self.output_size), 1]))
